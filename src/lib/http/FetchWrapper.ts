...
export default class FetchWrapper implements HttpInterface {
  ...
  get<T>(...arg: any): RequestResponse<T>
  get<T>(
    ...arg: any
  ): RequestResponse<T> {
  ...
  processPostOverloading<T = any>(...arg: any): RequestResponse<T> {
    ...
    if (typeof arg[0] === "string" && arg[1]) {
      ...
      // TODO: make isHttpMethod
      if (typeof arg[1] === "string" && Object.keys(HttpMethod).includes(arg[1].toUpperCase())) {
        return this.postBiparamType<T>(arg[0], undefined, arg[1] as HttpMethod)
      }
  ...
  request<T = any>(url: string, init: RequestInit & { data?: { [index: string]: any } }): RequestResponse<T> {
    ...
    if (contentType === ContentType.MULTIPART || contentType === ContentType.URLENCODED) {
  ...
  async processResponse<T = any>(r: Response) {

    return r.json() as RequestResponse<T>
  }
  ...
  private async postBiparamType<T = any>(url: string, data: Data | undefined, method: HttpMethod) {
